...
=begin  
  def test_MintBootstrap
    ...
    assert pair.balanceOf( address( ALICE )) == 1.ether - 1000
...
###################
#  start new part ii tests

def test_SwapBasicScenario
  token0, token1, pair =  _setup_contracts
...
def test_SwapBasicScenarioReverseDirection
  token0, token1, pair =  _setup_contracts
